function nROUSE_simple1

% assignments for visual layer

    o.VPR=1;   % visual prime
    o.VTR=2;   % visual target
    o.VMK=3;   % visual mask
    o.VTRC=4;  % visual target choice
    o.VFLC=5;  % visual foil choice
...
% parameters
    ...
    N=0.0099617;    % noise constant
    L=.15;          % constant leak current
    D=.324;         % synaptic depletion rate
    R=.022;         % recovery rate
    I=1.1063;       % inhibition constant
    T=.15;          % activation threshold
    S=[0.022305, 0.068730, .015];    % integration time constants at each level
    ...
    for cd=1:2      % cd is a step through index for conditions    
        ...
        [accs(:,cd)]=simulate;  % run all prime durations and return accuracy
    end
    ...
    function [acc]=simulate
        ...
        for pd=1:size(durations,2)      % pd is a step through index for prime durations
            ...
            Latency=zeros(1,2);     % identification latencies for each choice
            ...
            % calculate accuracy
            mean_diff=Latency(FOIL)-Latency(TARG);
            var_diff=N.*sum(Latency.^2);
            acc(pd,1)=1-normcdf(0,mean_diff,var_diff.^.5);
            
            if Latency(TARG)==0 && Latency(FOIL)>0      % target never launched
                acc(pd,1)=0;
            elseif Latency(TARG)>0 && Latency(FOIL)==0  % foil never launched
                acc(pd,1)=1;
            elseif Latency(TARG)==0 && Latency(FOIL)==0 % neither launched
                acc(pd,1)=.5;
            end
        end
    end
